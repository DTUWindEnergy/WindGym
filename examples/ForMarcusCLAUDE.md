# Derating in WindGym — the surrogate approach

*Companion note to `Example 6 Derating Surrogate.ipynb`.*

## What derating does, physically

Derating (downregulation) makes a turbine produce a fraction `1 − derate` of
its available power. The controller achieves this by moving the operating
point (pitch + TSR), and this **also reduces the thrust coefficient** — so a
...
The DTU 10MW power/ct behaviour under derating is tabulated by
[hawcpowerctcurvegenerator](https://gitlab.windenergy.dtu.dk/TOPFARM/cuttingedge/pywake/hawcpowerctcurvegenerator)
into `ws_derating_yaw_surrogatefull.nc` (66 MB, not shipped with WindGym):

- **Dimensions**: wind speed 0–25 m/s, yaw ±40°, derating 0–0.8.
- **Variables**: power, ct (plus the pitch/TSR operating points and cp).
...
See the notebook's Section 1 for the complete factory function.
...
Info dicts expose `"derate agent"` (applied values) and `"derate measured"`.
...
the upstream loss. An optional `derate_penalty` (change-based) exists to
...
without a surrogate (e.g. SWT-2.3 in `Example 5 Derating.ipynb`), applied
...
- `examples/Example 6 Derating Surrogate.ipynb` — this approach, end to end
  (turbine factory, physics plots, env usage, rendered gifs in
  `examples/images/`).
- `examples/Example 5 Derating.ipynb` — legacy power-curve-inversion approach.
