...
class Mes():
    ...
    def __init__(self, current=True, rolling_mean=False, history_N=3, history_length=100, window_length=5):
        ...
        self.history_N = history_N              #Number of rolling windows to use for the rolling mean
        ...
        if self.rolling_mean:
            ...
            for i in range(self.history_N):
                if i == 0:
                    ...
                    result = list(itertools.islice(self.measurements, self.history_length-self.window_length, self.history_length ))

                elif i == self.history_N-1:
                    #Gets the oldest values
                    result = list(itertools.islice(self.measurements, 0, self.window_length)) 

                else:
                    #Gets middle values
                    mes_place = self.history_length//(self.history_N-1) * i #Calculates where the window be middled 
                    result = list(itertools.islice(self.measurements, int((mes_place-self.window_length/2)), int(mes_place+self.window_length/2)))
...
class turb_mes():
    ...
    def __init__(self, ws_current=False, ws_rolling_mean=True, ws_history_N=1, ws_history_length=10, ws_window_length=10,
                       ...
                       yaw_current=False, yaw_rolling_mean=True, yaw_history_N=2, yaw_history_length=30, yaw_window_length=1,
                       power_current=False, power_rolling_mean=True, power_history_N=1, power_history_length=10, power_window_length=10,
                       ws_min:float = 7.0, ws_max:float = 20.0, 
                       wd_min:float = 270.0, wd_max:float = 360.0,
                       yaw_min = -45, yaw_max = 45,
                       TI_min = 0.00, TI_max = 0.50, include_TI = True,
                       power_max = 2000000, #2 MW
                       ):
        
        self.ws = Mes(current=ws_current, rolling_mean=ws_rolling_mean, history_N=ws_history_N, history_length=ws_history_length, window_length=ws_window_length)
        self.wd = Mes(current=wd_current, rolling_mean=wd_rolling_mean, history_N=wd_history_N, history_length=wd_history_length, window_length=wd_window_length)
        self.yaw = Mes(current=yaw_current, rolling_mean=yaw_rolling_mean, history_N=yaw_history_N, history_length=yaw_history_length, window_length=yaw_window_length)
        self.power = Mes(current=power_current, rolling_mean=power_rolling_mean, history_N=power_history_N, history_length=power_history_length, window_length=power_window_length)
    ...
    def get_yaw(self, scaled=False):
        ...
        if scaled:
            #Scale the measurements
            return self._scale_val(self.yaw.get_measurements(), self.yaw_min, self.yaw_max)
        else:
    ...
    def get_power(self, scaled=False):
        ...
        if scaled:
            #Scale the measurements
            return self._scale_val(self.power.get_measurements(), 0, self.power_max) #Min power is 0
        else:
    ...
    def get_measurements(self, scaled=False):
        ...
        if scaled:
            #Scale the measurements
            return np.concatenate([self._scale_val(self.get_ws(), self.ws_min, self.ws_max),  
            ...
            return np.concatenate([self.get_ws(), self.get_wd(), self.get_yaw(), self.get_TI(), self.get_power()])
...
class farm_mes(WindEnv):
    ...
    def __init__(self, n_turbines,
                 ...
                 farm_ws = True, farm_wd = True, farm_TI = False, farm_power = False,
                 ws_current=False, ws_rolling_mean=True, ws_history_N=1, ws_history_length=10, ws_window_length=10,
                 wd_current=False, wd_rolling_mean=True, wd_history_N=1, wd_history_length=10, wd_window_length=10,
                 yaw_current=False, yaw_rolling_mean=True, yaw_history_N=2, yaw_history_length=30, yaw_window_length=1,
                 power_current=False, power_rolling_mean=True, power_history_N=1, power_history_length=10, power_window_length=10,
                 ws_min:float = 7.0, ws_max:float = 20.0, 
        ...
        self.farm_observed_variables = farm_ws * (ws_current + ws_rolling_mean * ws_history_N) + farm_wd * (wd_current + wd_rolling_mean * wd_history_N)  + farm_TI + farm_power * (power_current + power_rolling_mean * power_history_N)


        #For each turbine, create a class of turbine measurements
        for _ in range(n_turbines):
            self.turb_mes.append(turb_mes(ws_current and turb_ws, ws_rolling_mean and turb_ws, ws_history_N, ws_history_length, ws_window_length,
                                          wd_current and turb_wd, wd_rolling_mean and turb_wd, wd_history_N, wd_history_length, wd_window_length,
                                          yaw_current, yaw_rolling_mean, yaw_history_N, yaw_history_length, yaw_window_length,
                                          power_current and turb_power, power_rolling_mean and turb_power, power_history_N, power_history_length, power_window_length,
                                          self.ws_min, self.ws_max, wd_min, wd_max, yaw_min, yaw_max, 
                                          TI_min, TI_max, turb_TI,
                                          power_max))
        ...
        self.farm_mes = turb_mes(ws_current and farm_ws, ws_rolling_mean and farm_ws, ws_history_N, ws_history_length, ws_window_length,
                                 wd_current and farm_wd, wd_rolling_mean and farm_wd, wd_history_N, wd_history_length, wd_window_length,
                                 yaw_current, yaw_rolling_mean, yaw_history_N, yaw_history_length, yaw_window_length,
    ...
    def add_power(self, measurement):
        ...
        if self.farm_power:
            self.farm_mes.add_power(np.sum(measurement))  #Instead of mean, we sum the power
    ...
    def add_measurements(self, ws, wd, yaws, powers):
        ...
        for turb, speed, direction, yaw, power in zip(self.turb_mes, ws, wd, yaws, powers):
            turb.add_ws(speed)
    ...
    def get_ws_turb(self, scaled=False):
        #get the ws measurements
        return np.array([turb.get_ws(scaled=scaled) for turb in self.turb_mes]).flatten()
    ...
    def get_power_turb(self, scaled=False):
        #get the power measurements
        return np.array([turb.get_power(scaled=scaled) for turb in self.turb_mes]).flatten()

    def get_power_farm(self, scaled=False):
        #get the power measurements
        if scaled:
            #Scale the measurements
            return self._scale_val(self.farm_mes.get_power(), 0, self.power_max*self.n_turbines) #Min power is 0, max is the sum of all the turbines
        else:
            return self.farm_mes.get_power()

    def get_wd_turb(self, scaled=False):
        #get the wd measurements
        return np.array([turb.get_wd(scaled=scaled) for turb in self.turb_mes]).flatten()
    ...
    def get_TI_turb(self, scaled=False):
        ...
        return np.array([turb.calc_TI(scaled=scaled) for turb in self.turb_mes]).flatten()
    ...
    def get_measurements(self, scaled=False):
        ...
        turb_measurements = np.array([turb.get_measurements(scaled=scaled) for turb in self.turb_mes]).flatten()
