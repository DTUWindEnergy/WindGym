...
def test_power_optimization():
    ...
    def compute_power(yaws):
        return wf_model(x=x_pos, y=y_pos, ws=wind_speed, wd=wind_dir, TI=TI, tilt=0, yaw=yaws).Power.sum()
    
    # Compute nominal power
    nominal_power = compute_power([30, 0])
    
    # Run optimization
    agent = PyWakeAgent(x_pos=x_pos, y_pos=y_pos, wind_speed=wind_speed, wind_dir=wind_dir, TI=TI)
    agent.optimize()
